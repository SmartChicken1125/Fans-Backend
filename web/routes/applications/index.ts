...
export default async function routes(fastify: FastifyTypebox) {
	...
	fastify.get<{}>(
		...
		async (request, reply) => {
			...
			const result: GetApplicationsRespBody = {
				applications: rows.map((a) => ModelConverter.toIApplication(a)),
			};
	...
	fastify.get<{ Params: AppIdParam }>(
		...
		async (request, reply) => {
			...
			if (!row) {
				return reply.sendError(APIErrors.ITEM_NOT_FOUND("Application"));
			}
	...
	fastify.delete<{ Params: AppIdParam }>(
		...
		async (request, reply) => {
			...
			if (result) {
				...
				return reply.sendError(APIErrors.APPLICATION_DELETION_FAILED);
			}
		},
	);
